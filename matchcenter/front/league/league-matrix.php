<?php
/**
 * Template part for League matrix block SNS.
 *
 * @author      Alex Torbeev
 * @category    Template
...
$teams = array(); //array_filter( get_post_meta( $id, 'sp_team', false ) );
...
<div class="sp-table-wrapper">
	<table class="sp-event-matrix sp-data-table sp_matrix_rows<?php if ( $scrollable ) { ?> sp-scrollable-table<?php } ?>">
		<thead>
			...
			<?php foreach ( $rows as $team_id => $row ) { ?>
				<th class="sp-event-matrix-label">
					...
					if ( has_post_thumbnail( $team_id ) ) {
						$name = '<span class="sp-event-matrix-team-logo">' . sp_get_logo( $team_id, 'sportspress-fit-mini', array( 'itemprop' => 'url' ) ) . '</span>';
					} else {
						...
						<?php } else { ?>
							<td class="sp-event-matrix-cell">
								...
								foreach ( $events as $event ) {
									$event_model = new SP_SNS_Event($event);
									$outcome = sp_array_value( sp_get_outcome( $event ), $home_id, null );
									$result = implode( '-', sp_get_main_results_or_date( $event, $date_format ) );
									$color = sp_array_value( $colors, $outcome, '#1d87f1' );
									$event_title = $event_model->team_home->post->post_title . ' ' . $result . ' ' . $event_model->team_away->post->post_title;

									//if ( $link_events ) {
										if ( $outcome ) {
											$result = '<a class="sp-event-matrix-event sp-event-matrix-event-' . $outcome . '" style="background-color: ' . $color . ';" href="' . get_post_permalink( $event, false, true ) . '" title="' . $event_title . '">' . $result . '</a>';
										} else {
											$result = '<a class="sp-event-matrix-future-event" href="' . get_post_permalink( $event, false, true ) . '" title="' . $event_title . '">' . '&nbsp;' . '</a>';
										}
/*									} else {
